...
const PaymentRequestScreen = ({navigation}) => {
  ...
  const getListViewItem = async item => {
    ...
    Alert.alert('Recharge amount', `K ${item.deposit_amount.toFixed(2)}`, [
      {
        text: 'Close',
        style: 'cancel',
      },
          ...
          <TouchableOpacity>
            <View
              ...
              <Text style={{paddingLeft: 10}}>
                Mobile Number :{' '}
                {userData.id === item.sender.id
                  ? item.receiver.mobile_number
                  : item.sender.mobile_number}
              </Text>
              <Text style={{paddingLeft: 10}}>
                {userData.id === item.sender.id ? 'Outcome' : 'Income'} Amount :{' '}
                {Theme.currency} {item.transaction_amount.toFixed(2)}
              </Text>
            </View>
          </TouchableOpacity>
        )}
