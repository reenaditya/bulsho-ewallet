...
import { resendOTP,verifyOTP } from '../../services/RegisterService';
...
export default function OTP ({navigation}){
	...
	const nextScreen = async () => {
		...
		if (otpcombine.length == 4) {
			...
			await verifyOTP({mobile_number:mobileNumber,otp:otpcombine})
			.then(res => {
		...
		<View style={styles.container}>
            <ScrollView keyboardShouldPersistTaps={'handled'} contentContainerStyle={{alignItems: 'center' }}>
		 	...
		 	<View style={styles.subHeading}>
		 		...
		 		<Text style={styles.numberShow}>{mobileNumber}</Text>
	      		<View
	            style={{
	            	flexDirection: 'row',
	            	width: '90%',
      			...
      			<TouchableOpacity>
			        <Text style={styles.signUpButton} onPress={() => navigation.goBack()}> Edit my Mobile Number   
        			</Text>
      			</TouchableOpacity>
